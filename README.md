...
This repo is the product: public Markdown and `package.xml` templates for moving an **existing** Agentforce implementation from a work sandbox to production with Salesforce CLI. Consultants and FDEs use it on a short handoff. It is not a customer ALM rollout.

GitHub is home. There is no Drive, second wiki, or SFCI overlay. Do not expand this repo into a pipeline product.

Guides, the index, and nav live in [docs](docs) ([docs/index.md](docs/index.md), [docs/meta.json](docs/meta.json)). Starting `package.xml` files live in [manifests](manifests). The staged deploy skeleton is [templates/deploy.mjs](templates/deploy.mjs).
...
| Engineer | Ship the Now item (Grok Build / Grok 4.6 xhigh unless Product or CoS assigns otherwise) |
...
- Most work stays in the Agentforce Docs Team room.
- One Now item → one PR.
